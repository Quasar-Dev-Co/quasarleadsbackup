...
/**
 * Saves Dutch final template as DRAFT - DOES NOT AUTO-SEND
 * User must manually review and approve in the UI before sending
 */
...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa;">
    ...
    // Create the new AI response record as DRAFT
    const newAIResponse = new AIResponse({
      ...
      responseType: 'final_template',
      reasoning: `Beautiful Dutch final template for 3rd+ reply (conversation count: ${email.conversationCount || 3}) - saved as DRAFT for user approval`,
      userId: userId,
      autoGenerated: true // Mark as auto-generated by cron
    });
    
    await newAIResponse.save();
    console.log(`💾 Dutch final template saved as DRAFT for email: ${email.subject}`);

    // Update incoming email status to show it has been processed (but not sent)
    await IncomingEmail.findByIdAndUpdate((email as any)._id, {
      status: 'pending_ai', // Mark as pending user review
      processedAt: new Date()
    });
    
    console.log(`✅ Dutch final template saved as DRAFT for: ${email.leadEmail} - waiting for user approval`);
    return true;
...
/**
 * Saves AI response as DRAFT - DOES NOT AUTO-SEND
 * User must manually review and approve in the UI before sending
 */
async function saveAsDraft(email: any, reply: { subject: string, content: string }, reasoning: string, userId?: string): Promise<boolean> {
  try {
    ...
    await newAIResponse.save();
    console.log(`💾 AI response saved as DRAFT for email: ${email.subject}`);
    ...
    console.log(`✅ AI response saved as DRAFT for: ${email.leadEmail} - waiting for user approval`);
    return true;
 ...
 * THIS IS ONLY CALLED WHEN USER MANUALLY CLICKS SEND IN THE UI
 */
  ...
  try {
    ...
    for (const email of unreadEmails) {
      try {
        ...
        if (isThirdReply) {
          console.log(`🇳🇱 Generating Dutch final template for ${email.leadEmail} (3rd+ reply) - SAVING AS DRAFT`);
          
          // Generate Dutch final template and save as DRAFT (DO NOT AUTO-SEND)
          ...
          if (success) {
            ...
            errors++;
            console.log(`❌ Failed to save Dutch final template draft for: ${email.leadEmail}`);
          }
      ...
      if (success) {
          sentReplies++;
          console.log(`✅ AI response saved as DRAFT for: ${email.leadEmail} from user: ${userEmail}`);
      } else {
          errors++;
          console.log(`❌ Failed to save draft for: ${email.leadEmail} for user: ${userEmail}`);
        }
